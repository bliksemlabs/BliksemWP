...
// instead of 180, but keeping both axes at the same scale enables efficent distance calculations.
// In any case the extra Y resolution is unnecessary, since 1 brad is already just under 1cm.
...
If we map zero brads to 0 longitude and xvalues are not scaled, unsigned and signed brads have identical binary representations.
The main advantage of using unsigned brads is that is prevents hashgrids from "reflecting" around the origin when normal division is performed.
The unsigned conversion could be performed within the hashgrid itself, and binning could be performed with masks (shift out bits within cell, zero out high bits, rest is cell number).
If the x values are stored pre-scaled we will not get proper wrapping at the date line, but bins will be of a consistent size.
If the x values are stored unscaled then we will get seamless wrapping at the date line, but bins will not be of a consistent size.
Not all architectures will use twos complement signed integers... which don't?
In order to efficiently calculate distances, we need the x and y coordinates to be in the same units / at the same scale.
We can either sacrifice vertical resolution and store both coordinates at the x scale, or perform an additional multiply operation at every distance calculation to restore the y coordinate to the same scale.
...
// It is implemented as the square of the distance in latitude-scaled/projected binary radians.
double coord_ersatz_distance (coord_t *c1, coord_t *c2) {
    double dx = c2->x - c1->x;
    double dy = c2->y - c1->y;
    return dx * dx + dy * dy;
} // rename to coord_distance_ersatz, add ersatz_from_meters and meters_from_ersatz

// Returns the equivalent ersatz distance for a given number of meters, for distance comparisons.
double ersatz_distance (double meters) {
...
double latlon_distance_meters (latlon_t *ll1, latlon_t *ll2) {
    // Rather than finding and using the average latitude for longitude scaling, or scaling
    // the two latlons separately, just convert the to the internal projected representation.
    coord_t c1, c2;
